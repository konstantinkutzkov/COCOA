...
class StackLevel {
  ...
  // clause branching: true if this level branches on a clause
  // (subtraction instead of addition for model count)
...
public:
  ...
  StackLevel(unsigned super_comp, unsigned lit_stack_ofs,
      unsigned comp_stack_ofs, bool is_clause_branch = false,
      unsigned clause_ofs = 0) :
      ...
      clause_branch_(is_clause_branch),
      clause_branch_ofs_(clause_ofs) {
  ...
  bool isClauseBranch() const {
    return clause_branch_;
  }

  unsigned clauseBranchOfs() const {
    return clause_branch_ofs_;
  }
  ...
  const mpz_class getTotalModelCount() const {
    if (clause_branch_) {
      ...
      if (branch_found_unsat_[0])
        return mpz_class(0);  // F\{C} UNSAT => F UNSAT
      if (branch_found_unsat_[1])
        return branch_model_count_[0];  // ¬C infeasible => all of F\{C} satisfies C
      mpz_class result = branch_model_count_[0] - branch_model_count_[1];
      assert(result >= 0);
      return result;
    }
