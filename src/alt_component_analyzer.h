...
public:
  ...
  void setupAnalysisContext(StackLevel &top, Component & super_comp){
     ...
     for (auto itCl = super_comp.clsBegin(); *itCl != clsSENTINEL; itCl++) {
         if (removed_clauses_ && removed_clauses_->count(clause_id_to_ofs_[*itCl]))
             continue;
  ...
  // ClauseIndex -> ClauseOfs mapping (built during initialize)
  ClauseOfs clauseOfsOf(ClauseIndex id) const {
    assert(id < clause_id_to_ofs_.size());
    return clause_id_to_ofs_[id];
  }
...
private:
  ...
  // maps ClauseIndex -> ClauseOfs (offset in literal pool)
  vector<ClauseOfs> clause_id_to_ofs_;
