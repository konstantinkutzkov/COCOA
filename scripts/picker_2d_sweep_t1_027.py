#!/usr/bin/env python3
"""
2D picker-parameter sweep on t1_027 only.
...
Budget: 60s per run. Expected count: 1115259056499565 (from benchmark_log).

Writes results to picker_2d_sweep_t1_027_results.csv (incremental).

Run from sharpsat-separator/ directory:
    python3 scripts/picker_2d_sweep_t1_027.py
"""
import csv
import re
import subprocess
...
def main():
    ...
    out_path = Path(__file__).parent / "picker_2d_sweep_t1_027_results.csv"
    runs = [(wl, av) for wl in WL_VALUES for av in ALPHA_VAR_VALUES]
    print(f"2D picker sweep on {CNF}: {len(runs)} runs, "
          f"budget={BUDGET_S:.0f}s each", flush=True)
