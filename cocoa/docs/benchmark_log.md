...
## mc2026_track1_169 — ✅ RESOLVED: cocoa-sep-cascade undercount root-caused (learned-clause cache pollution) + fixed (`-cachePurge 1 -provLocalTaint`) + full-pipeline-revalidated; sep-cascade un-quarantined. Count = 2,337,406,655,942,795,198,464 (now COCOA-solved, ganak-verified). [Original incident header: ⚠️ SOUNDNESS MISMATCH: cocoa-sep-cascade UNDERCOUNT vs ganak — sweep PAUSED, sep-cascade quarantined]
...
**✅ FIX VALIDATED — STAGE-1 SNAP TEST PASSED (2026-06-13, commits 8d967b2+cfc97bb):** the `-cachePurge` defense was implemented (journal + purge-on-failed-arm, default 0) and run on the verbatim incident configuration with `-cachePurge 1`: **count = 2,337,406,655,942,795,198,464 — EXACTLY ganak's oracle, on the instance+config that produced the wrong count.** Root cause (learned-clause cache pollution) thereby confirmed AND cured in one experiment. Cost of soundness on this worst-case instance: 2244s vs the buggy 1435s (**1.56×**); purge activity was massive — 1,426,152 purge events erasing 7.26M L2 + 0.83M L1 entries (9.55M total stores vs the incident's 4.43M — purged work recomputed and re-stored); journal peaked at 67 MB (cap 2 GB, 0 overflows), 0 mark imbalances. The conservative Level-0 purge leaves clear headroom for the taint-bit (Level-1) refinement — journal only stores whose subtree actually fired a learned clause. Mode-0 control run (must reproduce the wrong count bit-for-bit) and mode-2 exposure measurement follow.

**✅ MODE-0 CONTROL PASSED (same day):** new binary, `-cachePurge 0` → count **2337405402708146114560** (= the incident count exactly) with **byte-identical cache counters** (l2_stores=4429506, l2_hits=1773037, l1_stores=516617, l1_hits=2363215), purge machinery fully inert (all stats 0, journal_peak=0), 1447.8s vs 1435.1s (~0.9% wall noise); per-minute pct trajectory retraced the incident sample-for-sample. The fix-off binary is behaviorally identical ⇒ run 1's cure is attributable to the purge alone. *(Theory addendum from the same session's discussion: the owner's proposed strict learned-clause gate — block any clause with ANY variable outside the component, assigned or not — is provably sound by restriction-of-resolution + component connectivity, and is a second independent fix with the opposite cost profile [forfeits trail-residual CDCL pruning instead of cache contents]; candidate for an A/B against -cachePurge 1. The bug's firing channel, precisely: trail-falsified outside literals make a learned clause operate as a phantom local clause over component vars, invisible to the originals-only cache key.)*

**✅ MODE-2 MEASUREMENT (Stage 1 run 3, same day) — the purge's price tag on 169:** non-interference confirmed (wrong count + byte-identical counters reproduced; +2% overhead from the dead-set probes). Exposure: **2,298,539 distinct L2 + 298,041 L1 entries (~52% of all stores) would be purged; 898,927 of 1,773,037 L2 hits (50.7%) and 1,505,460 of 2,363,215 L1 hits (63.7%) landed on them; avg dead-hit component ~87 vars.** HALF of 169's cache value rides on purge-suspect entries — the anatomy of mode 1's 1.56×. Conclusion: -cachePurge 1 is sound and validated but expensive on heavy-reuse instances; build the **Level-1 taint bit** (journal only stores whose subtree actually fired a learned clause, + a mode-2 taint-rate counter) BEFORE any default flip. Stage 1 verdict: fix proven (run 1), binary behavior-identical at mode 0 (run 2), cost quantified (run 3).

**LEVEL-1 TAINT REFINEMENT (commit e6f8ace) + MEASUREMENT (same day):** taint = a non-local ("phantom") learned/redundant clause fired in the solve's subtree OR a tainted cache entry was consumed (taint propagates through hits; entries carry the bit; L1 copies inherit; outermost-success PROMOTION clears certified-pure survivors). Journal/purge now covers ONLY tainted stores; fully-in-mask learned firings are exempt (provably locally entailed: restriction-of-resolution + component connectivity). Smoke: purges 147→79 on 137 (40% of stores pure). **169 mode-2 measurement: tainted_stores=4,944,200 vs pure_stores=1,922 (99.96% tainted; 1.89M phantom firings; taint overhead +1.2%)** — on 169 the taint bit recovers ~nothing (phantom-saturated instance); the Level-0 cost (~1.56×) stands there. Efficiency levers remaining for 169-class: the owner's strict gate (block trail-spanning learned clauses in components: full cache power, weaker CDCL — A/B candidate) and provenance/tier-2 (could-fire context keys). Taint-gated mode-1 snap+speed rerun (run B) follows.
...
**GATE EXPERIMENT (same day, killed at +15 min):** `-learnedLocalOnly -cachePurge 1` on 169 (commit 2dfd9e9): pct 4.7% at +15 min vs the crude purge's ~21% — **on phantom-saturated instances the phantoms are load-bearing (1.9M prunings); blocking them inflates the search far more than the preserved cache repays. Gate LOSES to purge on 169-class.** Killed at the agreed checkpoint. Verdict: gate stays in the toolbox for low-phantom instance profiles (portfolio A/B pending); crude purge remains the best validated sound config for 169-class. Note: the taint-mode-1 run (also killed early) trailed the crude purge's curve unexpectedly — suspicion: the outermost-success PROMOTION walk (O(journal) finds) may be a pessimization on huge journals; to investigate before any default decision.

**PROVENANCE-LOCALITY MEASUREMENT on 169 (2026-06-13, -cachePurge 2 -measureProvLocal; commit dcb8164+):** σ-aware validator (BFS provenance to original leaves, σ-restrict each, require surviving vars in component). Non-interfering (wrong count reproduced). Result: **phantom_long_firings=20,235,429; provlocal=19,582,140 (96.8% certified locally entailed); leaf_outside=486,703 (2.4% genuinely non-local); no_mask=166,586 (0.8% root-scope); UNRECORDED=0 (provenance coverage complete).** Vindicates the hypothesis: ~97% of phantom firings are safe for their component; the undercount lives in the residual 3.2%. CAVEATS: (1) firing-EVENT rate, not per-solve — taint is per-solve (one non-local firing taints the whole solve), so cache recovery depends on how the 653k non-local firings scatter across solves (per-solve measurement needed, or measure directly by wiring); (2) validator un-memoized → +31% overhead here (1969s vs ~1500s baseline) — MUST memoize per (clause,component) before production wiring. NEXT: wire validator into taint decision (taint only on non-local firings) + memoize, rerun mode-1 on 169 — count MUST stay ganak's 2337406655942795198464 (soundness test; 169 is the catcher), and tainted_stores reduction + speedup is the payoff measurement.

**✅ LEVEL-2 (provenance-gated purge) WORKS — sound AND faster (2026-06-13, -provLocalTaint -cachePurge 1 on 169):** count = 2337406655942795198464 (ganak's EXACT oracle — SOUND on the catcher instance), **time 1773s vs crude purge 2244s (21% faster); soundness overhead 1.56×→1.24× (recovers ~57% of the purge's excess).** Amortized validator (provenance original-leaves computed once at clause birth, capped PROV_MAX_LEAVES=64; per-firing = σ-restriction scan). Stats: purged_l2=2.32M vs crude 7.26M (~5M entries spared, 68% fewer purges); pure_stores 1.20M + promoted 1.87M = ~3.07M pure (vs Level-1's 1922); taint_fires 906k vs Level-1's 1.89M. Residual taint breakdown: provfail_unrecorded=688,580 (PROV_MAX_LEAVES=64 cap overflow — HEADROOM: raising the cap should rescue much of this), leaf_outside=199,037 (genuinely non-local), no_mask=18,084 (root-scope). DECISION UPDATED: validator UN-SHELVED — Level-2 is a working sound optimization; next cheap win = raise PROV_MAX_LEAVES and re-measure.

**✅ LEVEL-2 CAP-RAISE — FLOOR REACHED (2026-06-13, -provLocalTaint -cachePurge 1 -provMaxLeaves 4096 on 169):** count = ganak's exact 2337406655942795198464 (SOUND), **1711s** (vs cap-64 1773s, crude 2244s, buggy 1435s) = **1.19× overhead, 66% of the purge's excess cost recovered.** `prov_max_leafset_seen=545` → 169's deepest derivation is 545 leaves, so the **default cap 1024 captures everything** (4096 ≡ 1024 here; O(1) hash-dedup makes the cap cheap). **unrecorded taint 688k→0** (all cap-overflow rescued). Residual taint 275k is GENUINE: leaf_outside=261k (real cross-component) + no_mask=14k (root-scope) — the floor of single-trace provenance (multi-trace = diminishing returns). purged_l2 1.74M (vs crude 7.26M, 76% fewer); pure_stores 1.98M + promoted 1.76M = 3.74M. **DECISION: Level-2 (`-cachePurge 1 -provLocalTaint`, default cap 1024) is the recommended sound standalone config. Soundness work DONE.**

**✅ FULL-PIPELINE END-TO-END VALIDATION — COCOA SOLVES 169 CORRECTLY AS LEADER (2026-06-13, runlogs/fullpipe_169_sound.log):** ran the *complete* pipeline (`pipeline.py --budget 3600`), not standalone, after wiring the sound flags into every COCOA archetype via `Archetype.COCOA_SOUND_FLAGS = ("-cachePurge","1","-provLocalTaint")` (single-point injection in `portfolio/race/archetypes.py`, default cap 1024). **Result: `{status: solved, winner: cocoa-sep-cascade, count: 2337406655942795198464, round: monitor, active_wall_s: 1706.3}`** — i.e. the **previously-quarantined `cocoa-sep-cascade` (the config that CAUSED the incident) now wins the 8→4→2→1 funnel AND lands ganak's exact oracle count**, no ganak handoff. Trajectory diverged from the buggy run exactly as predicted: buggy full-pipeline run (`runlogs/mc2026_t1_169.log`) finished early at active 1450s / ~7.04M decisions on the WRONG `...4560` undercount; this run did the full ~7.91M decisions over 1706s to reach the correct `...8464` (matches the standalone fix's ~8M/1711-1773s, NOT the buggy short-circuit). Funnel ranked sep-cascade top by closed_bits at every cut (r1 618.6 → r3 619.9; the high pct_lin is the known [[cocoa-metrics-mirage]] but it solved fast regardless). **sep-cascade UN-QUARANTINED — the fix repairs the exact config that broke, validated through the real funnel→monitor→completion machinery (not just standalone -cachePurge). The sweep is now sound by construction (every COCOA argv carries the fix), with ganak-verify retained as the belt-and-suspenders net.**

## mc2026_track1_171 — SOLVED, count = 1,825,157 (ganak-native handoff, ~36 min; ganak-oracle, no cross-check applies)

**2232v / 6498-cl, band=HIGH, log2_cost=106.3, sep_ratio=0.025 / sepsize=55 (TINY separator), root_sep=43, max_path=102, n_leaves=1035, arjun substantial=False.** First sweep instance post-soundness-fix; **all 8 COCOA configs carried `-cachePurge 1 -provLocalTaint` (sound by construction)** and sep-cascade raced normally (un-quarantined) — no mismatch, fix deployment is live and clean. SOLVED by **ganak-native via handoff**, **active_wall 2181s (~36 min)**, count = **1,825,157** (~1.8e6 = 2^20.80). Ganak's deterministic `--prob 0` IS the oracle and COCOA was bailed ⇒ no independent cross-check applies (cf 095/145-handoff precedent).

**A TEXTBOOK MIRAGE + VINDICATED HANDOFF.** Looked like the 145 family (band=high + *tiny* separator + arjun=False — 145 was COCOA-solvable), but here COCOA walled: the funnel kept the two cascades (nosep 2083.9 → leader, sep 2083.3), leader cocoa-nosep-cascade burst to closed_bits 2085 / **n_root=2090** then crept at a *decaying* rate (0.0116→0.0038 b/s). The **pct-ETA gate** fired 10 consecutive bails (active 200→291s; eta_pct 96min→3.1h ≫ 44min budget; rem≈5 bits = 32× more count-mass) → handoff at active 291s. **The mirage:** cb/n_root≈2090 implied a count near 2^2090, but the TRUE count is 2^20.8 — closed_bits measured a ~2090-bit *decomposition-search tree*, NOT count proximity (backbone/definability collapses it). COCOA would never have finished the 2090-bit grind efficiently; ganak's Arjun preprocessing + search cracked the small true count in ~36 min. Reinforces [[cocoa-metrics-mirage]] and validates the progressing-but-doomed pct-ETA handoff (cf 027/047/095). **Handoff cosmetic note:** ganak's `[ganak~live]` stats froze at cache_K=428/confl/s=14623.02 for ~21 min (active 360→1621s) — a stale-parse artifact (ganak `--verb 1` "c o" lines went quiet on a hard component; sampler echoed last value), NOT a stall — cache then jumped 428K→2975K and it solved. Cosmetic-only; consider a "stale-progress" indicator later.

## mc2026_track1_173 — SOLVED, count = 2,079,778 (ganak-native handoff, ganak 3.5 min; ganak-oracle, no cross-check applies)

**1105v / 3441-cl, band=HIGH, log2_cost=101.9, sep_ratio=0.043 / sepsize=47, root_sep=43, max_path=98, n_leaves=640, arjun substantial=False** — a smaller sibling of 171 (same band/small-sep/arjun=False family). Sound flags active on all 8 COCOA configs; sep-cascade raced normally. SOLVED by **ganak-native via handoff**, ganak **active 211s (~3.5 min)** of its 1723s budget, count = **2,079,778** (~2.08e6 = 2^20.99). Ganak's `--prob 0` oracle + COCOA bailed ⇒ no cross-check (095/171 precedent).

**ANOTHER MIRAGE — but COCOA got FAR closer than on 171 before the handoff.** Here the larger separator (0.043 vs 171's 0.025) flipped the funnel: the **separator/picker configs won** (8→4 kept unified-sep/reactive/plain/cache-max; cascades dropped — opposite of 171). Leader cocoa-unified-sep CLIMBED steadily (pct_lin 16%→25%→33% across rounds, then 33%→**75%** over ~17 min of monitoring) — a real finishing trajectory, NOT 171's flat creep — but the **"last-bit-is-hardest" tail bit** (cf 025/121): with **n_root≈1103.0, cb≈1102.58 → only ~0.42 bits remaining**, the closing rate decayed to ~0.01 bits/min, so the pct-ETA gate projected the final sliver would overrun budget → 10 bails → handoff at active 1201s/pct 75%. The mirage again: a ~1103-bit decomposition-search tree for a 2^21 count. Ganak's Arjun preprocessing cracked the small true count in 3.5 min — ~6× faster than COCOA's remaining tail. **Handoff vindicated even at 75% done** (closing 0.42 bits of count-mass slowly ≫ ganak's whole solve). **METRIC NOTE (learned this run):** `pct_lin% = 100·2^(−remaining_bits)` — the intuitive human progress measure (50%↔1 bit, 75%↔0.42 bits left); but its *slope* is deceptive in the tail (last 25% is log-deep), which is exactly why the forecaster's ETA runs in closed_bits (log) space, not pct space. Do NOT read n_root as n_vars (1105) — the forecaster's n_root here is ~1103.

## mc2026_track1_175 — KILLED (forecaster bug: rem≤0 "count complete" defeated the handoff) → FIXED; count PENDING

**18534v / 27748-cl (largest in this batch), band=HIGH, log2_cost=568, sep_ratio=0.0079 / sepsize=203 (tiny sep), max_path=564, n_leaves=8046, arjun substantial=TRUE** (first arjun=True of the batch). NOT a clean result — killed by hand after a forecaster bug, count not yet obtained.

**WHAT HAPPENED:** all 8 COCOA configs slammed into `closed_bits = n_root = 3973` within their first 60s window (arjun-style definability collapsed the effective search to ~3973 bits) — pct_lin=100%, vel=0 — but **none returned a count**: cb stayed pinned at 3973.0000 while decisions exploded (9M → 140M+). Leader cocoa-plain was kept on EVERY monitor recheck with verdict `keep :: remaining<=0 (count complete)` and **would have ridden to the 60-min wall → timeout, ganak never running** (despite arjun=True being a strong ganak-solve signal, cf 039/045). Killed manually at active ~900s.

**ROOT CAUSE (forecast.py:372, predict_cb_tree):** an unconditional `if remaining <= 0: keep ('count complete')`. But `n_root` is an ESTIMATE and can UNDERESTIMATE log2(count); cb saturating the estimated bound is NOT completion — the only ground truth for "done" is the solver RETURNING (which short-circuits the race). So the forecaster mis-read "stuck past an underestimated bound" as "done" and never evaluated the bail logic. Severe form of the known REM_FLOOR/pct-blind-patience open thread (121): never keep indefinitely on a bits-based "almost done" signal.

**✅ FIX (committed this session, forecast.py + tests/test_tree.py):** replaced the unconditional keep with a **flat-at-bound vs climbing-past-bound discriminator + completion grace**. At rem≤0: if cb is still CLIMBING past the bound (real progress) OR only briefly saturated (< COMPLETE_GRACE=60s, a genuine finisher's assembly window) → keep; if cb is FLAT at the bound past the grace with no return → **bail → handoff** (n_root underestimate). With the existing R3_CONSECUTIVE=10 debounce the handoff now fires ~160s past saturation instead of never. Regression tests added: #8 climbing-past-bound→keep (refined), #8b mc2026_175 flat-at-bound→bail, #8c brief-saturation→keep (grace respected). Full suite 15/15 tree + 90 passed. **175's count still PENDING** — re-run (ganak-direct, given arjun=True) is the next step.

## mc2026_track1_177 — BOTH-LOSE (killed at ~27 min; ganak 161-style useless-cache brute-force)

**13998v / 20941-cl, band=HIGH, log2_cost=474, sep_ratio=0.0094 / sepsize=188 (tiny sep), root_sep=123, max_path=470, n_leaves=6037, arjun substantial=False.** No count. **COCOA:** all 8 configs burst to closed_bits≈7097 (n_root≈7099, rem~2 bits, pct≤25%) in their first window then FROZE (vel=0); the two adaptive configs led on level with almost no search (~118k decisions) — a hard plateau ~2 bits short. Leader cocoa-adaptive frozen → **escape-tree frozen-path handoff fired correctly at active 240s** (normal rem>0 path; NOT the rem≤0 bug — good, confirms the standard handoff still works). **GANAK:** ran ~27 min on the handoff budget with the **mc2026_161 useless-cache signature** — `cache_K=0 cubes=0 confl/s=0.00` pinned for the entire run while RSS climbed/oscillated 2.8→14.8→12.3 GB (cache stores nothing useful, every lookup misses, 0 conflict pruning = brute-forcing a structurally-uncacheable residual). No intermediate count. Killed by hand at active ~1622s (the signature was unambiguous; ~17 min budget remained). **Profile = the band=high + arjun=False large-instance both-lose family** (029/033/081/083/161). Tiny separator gave COCOA only a shallow handle (froze ~2 bits short); ganak's preprocessing didn't crack the residual. Recorded as both-engines-lose, no count credited. (Note: this is a genuine both-lose, distinct from 175's forecaster-bug kill — here the handoff worked and ganak genuinely couldn't count it.)

## mc2026_track1_179 — SOLVED, count = 191,029,877,462,537,146,700,207,540,332,399,230,969,482,128,038,712,836,096,000 (cocoa-plain r1, 9.1s; ganak-verified)

**2224v / 3329-cl, band=HIGH, log2_cost=132.9, sep_ratio=0.0198 / sepsize=67, root_sep=62, max_path=129, n_leaves=1022, arjun substantial=True.** SOLVED by **cocoa-plain in round 1, 9.1s** (short-circuited — finished before any funnel cut), count = **191029877462537146700207540332399230969482128038712836096000** (~1.9e59 = 2^196.9); `ganak --prob 0` = same (**0.38s**, Arjun+GANAK) — **MATCH ✓**. **FIRST COCOA-leader win of the resumed sweep, and the FIRST ganak-verified COCOA count under the deployed sound config** (`-cachePurge 1 -provLocalTaint` active) — the fix produces the correct count, no mismatch (the soundness net held). band=high + arjun=True + small (2224v) = tractable for both; ganak's Arjun preprocessing made it ~24× faster (0.38s vs 9.1s), but COCOA solved it cleanly and correctly as leader. (Contrast 177: band=high + arjun=FALSE + 14k vars = both-lose. The arjun signal + scale again discriminate — cf 039/045/067.)

## mc2026_track1_181 — BOTH-LOSE (killed at ~27 min; ganak stuck in Arjun preprocessing)

**49807v / 75709-cl (LARGEST in the sweep), band=HIGH, log2_cost=1559, sep_ratio=0.011 / sepsize=560 (tiny sep), root_sep=560, max_path=1554, n_leaves=20871, arjun substantial=False.** No count. The 50k-var twin of 161 (51k, both-lose). **COCOA:** the cascade configs (nosep/sep-cascade) BURST to closed_bits 49804.6 / **pct_lin 77.5%** (rem~0.37 bits, n_root≈49805) in ~2400 decisions — the 145/175 tiny-sep burst — but then **plateaued** (r1→r2→r3: 77.51%→77.76%→77.77%, creep decaying to ~0); the separator/picker configs sat ~144 bits back (pct≈0). Leader cocoa-nosep-cascade → **pct-ETA gate bailed the stalled near-finisher at active 240s** (rem~0.36 > 0.2 sliver, rate→0 — the 121-style case the *existing* gate handles; the rem≤0 fix was NOT needed, confirming both paths work). **GANAK:** **stuck in Arjun/distill preprocessing for the entire ~27 min** — blank `[ganak~live]` stats throughout, RSS flat at 0.17→0.32→0.42→0.44 GB (memory-light preprocessing, never reached the counting phase), 100% CPU. Killed by hand (preprocessing would have eaten the budget without ever counting). **A SECOND both-lose failure mode for ganak at 50k scale:** distinct from 177's 161-style useless-cache-during-counting — here Arjun preprocessing itself doesn't terminate in budget. Recorded both-engines-lose, no count. Reinforces: **band=high + arjun=False + large (≥14k vars) is the reliable both-lose family** (161/177/181); COCOA's tiny-sep burst gets ~77% but stalls on the deep tail, ganak can't preprocess/count it.

## mc2026_track1_183 — BOTH-LOSE (killed at ~31 min; ganak stuck in Arjun preprocessing, like 181)

**2444v / 28462-cl (small but VERY DENSE, ~11.6 cl/var), band=HIGH, log2_cost=555, sep_ratio=0.091 / sepsize=225 (larger sep than 177/181), balance=0.10 (imbalanced cut), max_path=551, n_leaves=1370, arjun substantial=False.** No count. **COCOA:** cascade configs burst to closed_bits 2436 / pct_lin 3.3% (rem~4.9 bits, n_root≈2441) in ~14k decisions then froze (flat through r2/r3); separator/picker configs ground ~189 bits back (pct≈0). Leader cocoa-nosep-cascade → frozen-path handoff at active 240s. **GANAK:** **stuck in Arjun preprocessing the entire ~31 min** — blank `[ganak~live]` stats throughout, RSS flat ~0.32–0.33 GB (never reached counting), 100% CPU. Killed by hand. **Same preprocessing-stall both-lose as 181, but notable: it's only 2444 vars** — the DENSITY (28k clauses), not the var count, is what defeats Arjun here. So the both-lose family is broader than "large": **band=high + arjun=False + (large OR very dense) → ganak can't preprocess/count** (161/177/181 large; 183 small+dense). **SWEEP NOTE: 4 consecutive non-solves in the 175–183 band** (175 forecaster-bug-kill, 177/181/183 both-lose) — this region of track1 (high band, arjun=False, big/dense) is a hard wall for both engines; the wins were 169/171/173/179 (smaller or arjun=True/mirage).

## mc2026_track1_185 — BOTH-LOSE (killed at ~29 min; ganak preprocessing-stall, like 181/183)

**16704v / 25998-cl, band=HIGH, log2_cost=671, sep_ratio=0.016 / sepsize=270 (tiny sep), max_path=666, n_leaves=6986, arjun substantial=False.** No count. Textbook both-lose-family member. **COCOA:** cascades burst to closed_bits 16700.9 / pct_lin 47.5% (rem~1.08 bits) in ~7.7k decisions, slow-crept to 48.8% over ~3 min then STALLED (decisions exploded 39k→956k→5.35M, pct flat) → pct-ETA bail/handoff at active 360s. **GANAK:** preprocessing-stall — blank `[ganak~live]` stats the entire ~29 min, RSS low/slow-creeping 0.25→0.40 GB (Arjun never reached counting), killed by hand. **4th consecutive both-lose** (177/181/183/185), all band=high + arjun=False; the cascade burst gets 47–77% but stalls on the deep tail, ganak can't preprocess/count. The arjun=False + high-band region (track1 175–185, the large/dense instances) is a confirmed hard wall.

## mc2026_track1_187 — SOLVED, count = 134,217,728 = 2²⁷ (cocoa-unified-sep monitor, 354.8s; ganak-verified)

**2951v / 6095-cl, band=HIGH, log2_cost=152, sep_ratio=0.022 / sepsize=65, root_sep=44, max_path=147, n_leaves=1429, arjun substantial=True.** SOLVED by **cocoa-unified-sep in the monitor phase, active 354.8s (~6 min)**, count = **134,217,728 = 2²⁷**; `ganak --prob 0` = same (**13.77s**, Arjun+GANAK) — **MATCH ✓**. **Second COCOA-leader win of the resumed sweep + second ganak-verified count under the sound config** (after 179). Here — unlike the both-lose family — the separator/picker configs (unified-sep/cache-max/plain) led and **CLIMBED steadily** (unified-sep: rem 6.3→4.1→3.2→2.4 bits over the rounds, then pct 19.6→49.4%→finish), the cascades lagged. The arjun=True + small profile is tractable (cf 179): a ~2951-bit search tree resolving to a clean 2²⁷ count (mirage shape), closed COCOA-side. **Breaks the 177–185 both-lose streak — arjun=True is again the discriminator** (179/187 solved; 177/181/183/185 arjun=False both-lose).

## mc2026_track1_189 — BOTH-LOSE (killed at ~29 min; ganak preprocessing-stall, densest instance)

**6690v / 160057-cl (~24 cl/var — DENSEST in the sweep), band=HIGH, log2_cost=1267, sep_ratio=0.082 / sepsize=551, balance=0.079 (very imbalanced), max_path=1265, n_leaves=6749, arjun substantial=False.** No count. **COCOA:** cascades burst to closed_bits 6682 / pct_lin 53% (rem~0.91 bits) in ~2600 decisions then froze (flat r2/r3); separator/picker/adaptive configs ground ~445–473 bits back. Frozen-path handoff at active 240s. **GANAK:** preprocessing-stall — blank `[ganak~live]` stats the entire ~29 min, RSS flat ~0.6 GB (never reached counting), killed by hand. **5th consecutive both-lose** (177/181/183/185/189), all band=high + arjun=False. The extreme density (24 cl/var) is the 183-style Arjun-stall driver. **Confirmed rule: band=high + arjun=False + (large OR dense) ⇒ both-lose** — arjun=True (179/187) is the only reliable solve signal in the high band.

## mc2026_track1_191 — BOTH-LOSE (killed at ~30 min; ganak preprocessing-stall)

**30075v / 46455-cl, band=HIGH, log2_cost=1028, sep_ratio=0.0087 / sepsize=643 (tiny sep), max_path=1024, n_leaves=12558, arjun substantial=False.** No count. **COCOA:** cascades burst to closed_bits 30071.9 / pct_lin 46.5% (rem~1.10 bits) in ~3600 decisions, then crept only +0.024%→+0.014% over r2/r3 (decaying, NOT a finishing climb) → pct-ETA bail/handoff at active 240s. *(Genuinely uncertain at r1 — the cascade burst close, like 185 — but the decaying creep in r2/r3 confirmed the plateau, not a 187-style climb.)* **GANAK:** preprocessing-stall — blank `[ganak~live]` stats the entire ~30 min, RSS flat ~0.43 GB (never reached counting), killed by hand. **6th consecutive both-lose** (177/181/183/185/189/191), all band=high + arjun=False. The pattern is now extremely robust; arjun=True (179/187) remains the only high-band solve signal.

## mc2026_track1_193 — BOTH-LOSE (killed at ~27 min) — but a STRIKING near-miss: COCOA collapsed 121k vars to a single residual bit

**120936v / 1036073-cl (LARGEST by far: 121k vars, 1M clauses), band=HIGH, log2_cost=3921.7 (astronomical), sep_ratio=0.016 / sepsize=1969, root_sep=1547, max_path=3919, n_leaves=116538, arjun substantial=False.** No count. **COCOA — remarkable structure exploitation:** the separator configs closed cb 0→113,335 in one 60s window (vel≈11,300 b/s — the anonymized component cache collapsing ~116k mostly-isomorphic leaves), reaching **99.85% of the bits** (n_root≈113,501) but stalling at rem~166 bits; the **cascade configs did far better — collapsed the residual core to rem = EXACTLY 1.0 bit (pct_lin=50.000%, in ~168 decisions)**, then froze there (pinned at 50% through r2/r3/monitor, decisions trickling, cb flat) → pct-ETA/frozen handoff at active 300s. **GANAK:** preprocessing-stall — blank `[ganak~live]` stats the entire ~27 min, RSS flat ~0.62 GB (Arjun never got through the 1M clauses), killed by hand. **7th consecutive both-lose** (177–193, all band=high + arjun=False).

**THE 50%-EXACT SIGNATURE (theory, owner-prompted):** pct_lin=2^(cb−n_root)=0.5 ⟺ cb=n_root−1 ⟺ a top-level binary split into two equal-bound halves, one fully resolved, the sibling open. Two indistinguishable explanations: (a) **sibling is UNSAT** ⇒ true count is already known (= the resolved half) and COCOA is stuck doing what counters are worst at — *proving a large subspace UNSAT* (no models to memoize, must exhaust; a CDCL refutation might be short but the component-decomposition engine won't find it); (b) sibling is a second equally-hard SAT subproblem (true count ~2×). Can't distinguish mid-run from pct alone, and ganak (whose CDCL core might refute an UNSAT half) was itself stuck in preprocessing. **CANDIDATE FIX (this failure mode): a UNSAT-probe** — when a sibling branch stalls past a threshold, hand that subformula to a plain SAT solver; if UNSAT, the branch contributes 0 and the count terminates. Would convert this 121k-var near-miss into a solve IFF the residual is UNSAT. (Unverified premise; logged as an idea.) **Lesson reinforced (cocoa-metrics-mirage):** closed_bits can reach 99.85%+ — even rem=1.0 bit — on an UNSOLVABLE-in-budget instance; the last bit can be the whole difficulty.

## mc2026_track1_195 — BOTH-LOSE (COCOA cut in r1; ganak not run — 193 twin)

**121874v / 1035991-cl (122k vars, 1M clauses), band=HIGH, log2_cost=4001, sep_ratio=0.016 / sepsize=2268, root_sep=1537, max_path=3998, n_leaves=143112, arjun substantial=False.** No count. **Near-exact twin of 193.** COCOA reproduced 193's signature exactly: separator configs deep-grinding (cb 114033, rem~142 bits), and the **cascade configs collapsed to rem = EXACTLY 1.0 bit (pct_lin=50.000%, ~173 decisions)** — the same "stuck on the last bit / maybe-half-UNSAT" structure. **Cut in round 1** (during the COCOA funnel, before the ganak handoff) to save the ~30-min ganak preprocessing-stall already characterized on 193's identical twin — so **ganak was NOT run on 195** (both-lose recorded by twin-inference: COCOA stuck at rem=1.0 + ganak certain to preprocessing-stall on the 1M clauses as on 193). **8th both-lose** (177–195, all band=high + arjun=False). [Recorded as both-lose without an independent ganak run — the only such entry; justified by the exact-twin structural match to 193 + time economy at the owner's direction.]

## mc2026_track1_197 — BOTH-LOSE (COCOA cut in monitor; ganak not run — 193/195 triplet)

**91977v / 1046670-cl (92k vars, 1M clauses), band=HIGH, log2_cost=3805, sep_ratio=0.019 / sepsize=1885, root_sep=1634, max_path=3802, n_leaves=71125, arjun substantial=False.** No count. **Third member of the 193/195/197 huge-1M-clause triplet** — identical signature: cascade configs (nosep/sep) collapsed to closed_bits 91524 / **rem = exactly 1.0 bit (pct_lin≈50%)** and pinned there through r2/r3/monitor; separator configs deep-grinding (cb 91390, rem~135 bits). Cut in the monitor phase **before the ganak handoff fired** to skip the ~30-min preprocessing-stall characterized on 193 — **ganak NOT run on 197** (both-lose by triplet-inference: COCOA stuck at rem=1.0 + ganak certain to preprocessing-stall on 1M clauses). **9th consecutive both-lose** (177–197, all band=high + arjun=False).

## mc2026_track1_199 — BOTH-LOSE (ganak preprocessing-stall ~21 min; final sweep instance) + DECODED the family's application structure

**93601v / 1014592-cl (94k vars, 1M clauses), band=HIGH, log2_cost=3878, sep_ratio=0.024 / sepsize=2403, root_sep=1695, max_path=3873, n_leaves=95937, worst_leaf=3, arjun substantial=False.** No count. 4th of the huge-1M quartet (193/195/197/199). COCOA cascades collapsed to closed_bits 93300 / **rem = exactly 1.0 bit (pct_lin=50%)** then pinned (r1→monitor); separator configs deep-grinding (cb 93156). Handoff at active 300s → **ganak preprocessing-stall: blank `[ganak~live]` stats for ~21 min (active 1261s), RSS flat ~0.6 GB, never reached counting** → killed. **10th consecutive both-lose** (177–199). [Note: monitor notification delivery lagged real-time during this run — relayed "active Ns" trailed ganak's true position; killed cleanly, box verified idle.]

**DECODED THE APPLICATION STRUCTURE (owner-prompted, via raw-CNF analysis of 199/193):** the huge-1M family is a **templated finite-domain (size-7) selection/assignment counting problem**, NOT Sudoku. Fingerprint: (1) provenance stripped (`c t mc` only); (2) **periodic blocks of period 13** — each = `exactly-one-of-7` via the sequential/ladder encoding (7 primary vars + 6 aux chain vars: at-least-one len-7 clause `[k..k+6]` + binary AMO ladder `[-k,aux][-aux,aux+1]…`), ~7,200 base blocks; (3) clause-len histogram ~77% ternary + ~18-21% binary + a small equal-count len-4/5/6/7 tail (gadget signature); (4) WEAKLY constrained → count ≈ 2^(vars−few hundred), almost all assignments satisfy despite 8-11 cl/var; (5) a **~2,000-var backbone of high-degree hubs (deg ~270-680)** that *themselves* form exactly-one-of-7 blocks AND **fan out by step-13 implications** (`[-1229,1242][-1229,1255]…`) = monotone/ordering coupling threaded across many base blocks. **WHY IT'S THE EXACT WALL:** the ~7,200 base blocks are isomorphic & independent → anonymized component cache crushes them (→ rem=1 bit, worst_leaf=3); but the backbone is a **high-interaction-width connected core** — partial assignments yield genuinely *non-isomorphic* residuals (different boundary configs), so there's nothing to cache-merge (it's interaction width, NOT WL incompleteness — a complete iso test wouldn't help either). Beyond ALL portfolio exact methods: caching finds no recurrence, tree-DP needs treewidth≈2000 (2^2000), definability needs arjun=True (it's False). The count is genuinely #P-hard with no exploitable algebraic structure the tools detect. See [[mc2026-large-hardcore-family]].
