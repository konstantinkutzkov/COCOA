...
- **Tiny root separator (0.020) ⇏ COCOA-decomposable.** Structurally 127 is a near-twin of the 107/111/113/115/117 family (band=mid, sep_ratio≈0.020, arjun=True), all of which COCOA decompose-solved in round 1 (0.7–53s). The differentiator is the **deep dissection path**: nd_cost flagged **max_path=78** (the worst root-to-leaf chain accumulates 78 bits of separator cost) despite the cheap 52-var root cut and 1206 leaves. At runtime COCOA stalled ~30 bits short of n_root — stuck on the deep tail, not the shallow root. **Lesson: a tiny root_sep does NOT imply an easy tree — `max_path` (deepest accumulated separator cost) is the real predictor.**

So 127 is a genuine hard instance for the current portfolio: cleanly decomposable at the root but with a deep tail that neither COCOA's component search nor ganak's caching+preprocessing could close in an hour. **Count remains unknown.**
