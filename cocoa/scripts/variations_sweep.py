#!/usr/bin/env python3
"""
Generic 14-config sweep on a single instance.
Usage:
    python3 scripts/variations_sweep.py <cnf_basename> [budget_seconds]

Default budget: 60s per config. Output CSV: variations_<cnf_basename_stem>_results.csv
"""
import csv
import json
import os
...
def run_one(cnf: Path, name: str, flags: str, budget_s: float) -> dict:
    ...
    # Final per-tick values from trajectory
...
def main():
    if len(sys.argv) < 2:
        sys.exit("Usage: variations_sweep.py <cnf_basename> [budget_seconds]")
    ...
    out_path = Path(__file__).parent / f"variations_{stem}_results.csv"
