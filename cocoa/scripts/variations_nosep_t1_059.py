#!/usr/bin/env python3
"""
Focused mini-sweep on t1_059: do configs WITHOUT separator branching
do better than configs WITH it on this binary-heavy structured instance?

Hypothesis: METIS finds a 43-var separator on t1_059 that the size gate
accepts (min(0.3*264, 100) = 79 ≥ 43), so adaptive/picker configs branch
on those 43 structural vars first. But t1_059's 1392 binary clauses
mean BCP cascades are massive — the highest-BCP-cascade variables may
have zero overlap with the structural separator. Removing -sep -cb
should let the picker/adaptive score by their own metrics (which
include cascade) without being forced through the separator first.

Compares against the v2 12-config sweep results (saved separately).

Usage:
    python3 scripts/variations_nosep_t1_059.py [budget_seconds]

Default budget: 60s per config. Output CSV:
    variations_nosep_mc2025_track1_059_results.csv
"""
import csv
import os
import re
...
CNF = "mc2025_track1_059.cnf"
PROGRESS_INTERVAL_S = 5.0

# All configs OMIT -sep and -cb (i.e., no separator branching, no
# long-clause branching). Picker still picks vars via its scoring,
# adaptive still picks via failed-literal probing.
CONFIGS = [
    ("plain_noSep",
     "-rec -sepMode metis"),
    ("adaptive+wl2_noSep",
     "-rec -sepMode metis -adaptive -wlIter 2"),
    ("picker_cascade2_noSep",
     "-rec -sepMode metis -unifiedPicker -decomposeAfterK 1000 -cascadeW 2"),
    ("picker_cascade5d6_noSep",
     "-rec -sepMode metis -unifiedPicker -decomposeAfterK 1000 "
     "-cascadeW 5 -cascadeDepth 6"),
    ("picker_cascade10d9_noSep",
     "-rec -sepMode metis -unifiedPicker -decomposeAfterK 1000 "
...
def append_row(out_path, row, first):
...
def run_one(cnf, name, flags, budget_s):
    env = os.environ.copy()
...
def main():
    budget_s = float(sys.argv[1]) if len(sys.argv) > 1 else 60.0
    if not SHARPSAT.exists():
        sys.exit(f"sharpSAT not found: {SHARPSAT}")
    cnf_path = TEMP_CNF_DIR / CNF
    if not cnf_path.exists():
        sys.exit(f"CNF not found: {cnf_path}")
    out_path = Path(__file__).parent / f"variations_nosep_mc2025_track1_059_results.csv"
    print(f"{CNF}: {len(CONFIGS)} no-sep configs × {budget_s:.0f}s each "
